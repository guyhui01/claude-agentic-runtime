/**
 * Wiring for the WF-004 "AI Consulting Engagement" spine run (§2.4-B.4) —
 * assembles the real manifest (`WF_004_CONSEIL_MANIFEST`) into `SpineStep[]` via
 * `loadSpine`, then runs it with `runSpine` wired to the `query()` runner.
 *
 * Strictly modeled on `run-wf-003.ts` (same injection contract, same guards): the
 ...
 * (high-value strategic deliverables); route it via `model`.
 */
...
} from "./wf-004-conseil.js";

/** Call configuration error (insufficient parameters). */
export class Wf004ConfigError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "Wf004ConfigError";
  }
}

export interface RunWf004Options {
...
/**
 * Assembles the WF-004 backbone into `SpineStep[]` (real manifest + criteria
 * registry + resolver). Fail-closed via `loadSpine` (sidecar/registry cross-checks).
 */
export function assembleWf004Spine(
  sidecar: Sidecar,
  resolveAgent: AgentResolver,
): SpineStep[] {
  return loadSpine(
...
/**
 * Runs the WF-004 spine end to end: assembly then `runSpine` wired to the
 * `query()` runner. Returns the `SpineResult`, or throws on insufficient config /
 * an uncaught runner error (fail-closed).
 */
export async function runWf004(opts: RunWf004Options): Promise<SpineResult> {
  ...
  if (opts.resolveAgent) {
    ...
    if (root === undefined) {
      throw new Wf004ConfigError(
        "runWf004: provide `catalogRoot` (resolution via toAgentDefinition) or `resolveAgent`.",
      );
    }
  ...
  const steps = assembleWf004Spine(opts.sidecar, resolveAgent);
