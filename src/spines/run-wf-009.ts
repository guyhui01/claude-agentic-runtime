/**
 * Wiring for the WF-009 "IT/AI Recruitment" spine run (§2.4-B.4) — assembles the real
 * manifest (`WF_009_RECRUTEMENT_MANIFEST`) into `SpineStep[]` via `loadSpine`, then runs
 * it with `runSpine` wired to the `query()` runner.
 *
 ...
 * (operational HR deliverables); route it via `model`.
 */
...
} from "./wf-009-recrutement.js";

/** Call configuration error (insufficient parameters). */
export class Wf009ConfigError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "Wf009ConfigError";
  }
}

export interface RunWf009Options {
  /** Pinned catalog sidecar (real in prod, interim in tests). */
  sidecar: Sidecar;
  /** Recruitment context = the spine's initial input. */
...
/**
 * Assembles the WF-009 backbone into `SpineStep[]` (real manifest + criteria
 * registry + resolver). Fail-closed via `loadSpine` (sidecar/registry cross-checks).
 */
export function assembleWf009Spine(
  sidecar: Sidecar,
  resolveAgent: AgentResolver,
): SpineStep[] {
  return loadSpine(
...
/**
 * Runs the WF-009 spine end to end: assembly then `runSpine` wired to the
 * `query()` runner. Returns the `SpineResult`, or throws on insufficient config /
 * an uncaught runner error (fail-closed).
 */
export async function runWf009(opts: RunWf009Options): Promise<SpineResult> {
  ...
  if (opts.resolveAgent) {
    ...
    if (root === undefined) {
      throw new Wf009ConfigError(
        "runWf009: provide `catalogRoot` (resolution via toAgentDefinition) or `resolveAgent`.",
      );
    }
  ...
  const steps = assembleWf009Spine(opts.sidecar, resolveAgent);
