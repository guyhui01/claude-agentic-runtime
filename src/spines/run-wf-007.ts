/**
 * Wiring for the WF-007 "Client Engagement Onboarding D1-D5" spine run (§2.4-B.4) —
 * assembles the real manifest (`WF_007_ONBOARDING_MANIFEST`) into `SpineStep[]` via
 * `loadSpine`, then runs it with `runSpine` wired to the `query()` runner.
 *
 * Strictly modeled on `run-wf-005.ts` (same injection contract, same guards): the
...
} from "./wf-007-onboarding.js";

/** Call configuration error (insufficient parameters). */
export class Wf007ConfigError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "Wf007ConfigError";
  }
}

export interface RunWf007Options {
  /** Pinned catalog sidecar (real in prod, interim in tests). */
  sidecar: Sidecar;
  /** Engagement context = the spine's initial input. */
   ...
   * or a full model id). Only used when the resolver is derived from `catalogRoot`;
   * an injected `resolveAgent` owns its definitions entirely. Rationale: route live
   * runs to the user-designated performant model.
   */
  model?: string;
}

/**
 * Assembles the WF-007 backbone into `SpineStep[]` (real manifest + criteria
 * registry + resolver). Fail-closed via `loadSpine` (sidecar/registry cross-checks).
 */
export function assembleWf007Spine(
  sidecar: Sidecar,
  resolveAgent: AgentResolver,
): SpineStep[] {
  return loadSpine(
...
/**
 * Runs the WF-007 spine end to end: assembly then `runSpine` wired to the
 * `query()` runner. Returns the `SpineResult`, or throws on insufficient config /
 * an uncaught runner error (fail-closed).
 */
export async function runWf007(opts: RunWf007Options): Promise<SpineResult> {
  ...
  if (opts.resolveAgent) {
    ...
    if (root === undefined) {
      throw new Wf007ConfigError(
        "runWf007: provide `catalogRoot` (resolution via toAgentDefinition) or `resolveAgent`.",
      );
    }
  ...
  const steps = assembleWf007Spine(opts.sidecar, resolveAgent);
