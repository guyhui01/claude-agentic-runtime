...
function render(): string {
  ...
  let briefsWith = 0;
  // The denominator is the number of briefs actually PROBED, not the fixture
  // count: P18 carries no brief and is skipped, so `DISPATCH_FIXTURES.length`
  // would report a share of a population this loop never reads.
  let briefsProbed = 0;
  ...
  out.push("", `**${briefsWith} of ${briefsProbed} briefs carry any denial token at all.**`, "");
  return out.join("\n");
}
