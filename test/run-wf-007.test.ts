...
import { runWf007, Wf007ConfigError } from "../src/spines/run-wf-007.js";
...
/**
 * HERMETIC end-to-end test of the WF-007 run wiring (§2.4-B.4):
...
describe("runWf007 — WF-007 spine run wiring (§2.4-B.4)", () => {
  it("DoD-conformant outputs → spine completed via the real runner query()", async () => {
    const res = await runWf007({
      sidecar,
      resolveAgent,
      initialInput: { client: "Mid-cap industrial", type: "Scoping" },
  ...
  it("empty RACI (STEP-01) → failed at STEP-01", async () => {
  ...
  it("insufficient config: neither catalogRoot nor resolveAgent ⇒ Wf007ConfigError", async () => {
    await expect(
      runWf007({ sidecar, runnerDeps: { query: fakeQuery(happyOutputs), env: emptyEnv } }),
    ).rejects.toBeInstanceOf(Wf007ConfigError);
  });
});
