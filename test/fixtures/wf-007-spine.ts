/**
 * WF-007 spine-loading fixtures — SHARED interim sidecar + resolver + mock runner.
 *
 * Extracted from `spine-wf-007.test.ts` (verbatim, no behavior change) so the
 * hermetic spine test AND the discrimination audit (`spine-wf-007-discrimination.test.ts`)
...
/** The 4 WF-007 backbone agents (CHEF-PROJET-IA → BUSINESS-ANALYST → CHANGE-MANAGER → REDACTEUR-IA). */
export const wf007InterimSidecar: Sidecar = {
  schemaVersion: "1.0.0",
...
export const wf007ResolveAgent: AgentResolver = (asset): AgentDefinition => ({
