/**
 * WF-006 spine-loading fixtures — SHARED interim sidecar + resolver + mock runner.
 *
 * Extracted from `spine-wf-006.test.ts` (verbatim, no behavior change) so the
 * hermetic spine test AND the discrimination audit (`spine-wf-006-discrimination.test.ts`)
...
/** The 6 WF-006 backbone agents (CONSULTANT-IA → BUSINESS-ANALYST → AI-ARCHITECT → CHEF-PROJET-IA → FINANCIAL-ANALYST → REDACTEUR-IA). */
export const wf006InterimSidecar: Sidecar = {
  schemaVersion: "1.0.0",
...
export const wf006ResolveAgent: AgentResolver = (asset): AgentDefinition => ({
