/**
 * End-to-end proof (offline): the WF-004 spine is READY for the live run with the
 ...
 * `assembleWf004Spine` → `runSpine` (mocked runner, zero network). The
 * `createQueryRunner` wiring is covered by `run-wf-004.test.ts`.
 *
...
import { assembleWf004Spine } from "../src/spines/run-wf-004.js";
...
describe.skipIf(!HAVE_CATALOG)("WF-004 spine — REAL sidecar (ready for live run)", () => {
  it("the WF-004 backbone is resolvable from the real sidecar (inclusion)", () => {
    ...
    for (const id of WF_004_BACKBONE) {
  ...
  it("assembles + runs the spine with real prose → completed, 6 pass verdicts", async () => {
    const sc = loadSidecar(SIDECAR_PATH, CATALOG_ROOT);
    const steps = assembleWf004Spine(sc, (asset) => toAgentDefinition(asset, CATALOG_ROOT));
    ...
    const res = await runSpine(steps, mockRunner(wf004HappyOutputs), { client: "CAC40 insurer" });
...
describe.runIf(!HAVE_CATALOG)("WF-004 spine — real sidecar (skip)", () => {
