 ...
 *       STEP-04 Gherkin nominal/boundary/error typing is covered by the ADVISORY
 *       `qa-gherkin-3-types` (added 2026-08-23). ⚠️ It was briefly BLOCKING the same day: an
 *       independent audit caught that the sibling WF-001 resolves the SAME card sentence as
 *       advisory (`qa-cas-erreur-et-limite`), and that the line lives in `output_attendu`, not
 *       in `condition_passage` — so gating it inverted policy between two spines AND created a
 *       new false-halt for a valid-but-minimal QA deliverable. Both spines now share
 *       `coversCaseTypes` (normalized) and both schemas `enum` the vocabulary, so the policy is
 *       single-sourced and cannot diverge silently again. The rest of F4 stays ungated by
 *       decision (conditional / non-deterministic / needs an output field the frozen billed
 *       trace lacks — see the tracker's F4/F3(c)/F5 triage).
 * ─────────────────────────────────────────────────────────────────────────────
 */
