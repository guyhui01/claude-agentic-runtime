...
- **Project context**: POC `claude-agentic-runtime` — implements, in code terms, the non-optional-governance clause set in an upstream governance decision (DEC-0003, recorded outside this repo). This ADR *cites up* to that decision; it does not restate or crown it (repo boundary — `CLAUDE.md`).
...
DEC-0003 (an upstream governance decision, 2026-08-30) hardened the governance doctrine into a
...
Left undefined, "adapter layer" and "for flexibility" make the clauses cosmetic; the doctrine
defers their enforceable definition to a runtime ADR that implements the clause. This is that ADR.
Until the terms below are fixed, no change to this runtime can be objectively judged conformant or
in violation.
...
content is the customer's to define (the doctrine records this same carve-out). Clause (a) stays
...
- These three are recorded as a **tolerated, enumerated carve-out**, not silently ignored: they
  ...
  them through an adapter-owned type alias would close even that, but it belongs to the broader
  substrate-independence direction, beyond this ADR's scope. So it is a **named future step, not a
  debt this clause requires** — deferred deliberately, kept visible by the boundary test below.

### 4. Enforcement

Clause (b) is the one clause mechanically checkable at near-zero cost, and a principle nothing can
check is decorative. So it is enforced by a test, not by prose:
...
- **(a) Leave the clauses undefined** (ship the doctrine without operational definitions).
  Rejected: undefined clauses stay cosmetic and no feature can ever be found in violation.
- **(b) Define governance as configurable policy** (guards exposed as tunable/opt-out for
  ...
  effort closing (the `NO_CRITERIA` guard, 2026-08-30) and turns the fail-closed guarantee this
  runtime is built on into a setting. It was rejected upstream in the doctrine itself.
- **(c) Read clause (b) strictly enough to forbid type-only imports too**, and remediate the three
  ...
  front-runs the broader substrate-independence direction, beyond this ADR's scope. Recorded as a
  future step instead, not a debt.
...
- **The tolerated carve-out is a real, if type-only, vendor coupling in the core** — honest to
  record, and closing it is deferred to a future substrate-independence step, not to this ADR.
...
if **the broader substrate-independence direction is taken up** — at which point the tolerated
type-only carve-out (§3) stops being deferred and the core's type surface is routed through an
adapter-owned alias.

## Related

- **DEC-0003** — the upstream governance decision this ADR *implements* and cites up to; it sets
  the non-optional-governance constraint and the two violation clauses. This ADR gives clauses
  (a)/(b) and "adapter layer" their testable definitions.
