...
- ▶️ **§2.4-B.3 — run live de la spine (À FAIRE, sous condition)** : remplacer le runner mock par un adaptateur enveloppant `query()` et dérouler la spine pour de vrai.
  ...
  - ▶️ **Seul reste le RUN LIVE** : fournir le vrai `query()` (défaut SDK) + auth OAuth abonnement + catalogue checkout à v3.26.0, **sur accord explicite Guy + run observé** (auth + caps ci-dessous). Tout le reste est prêt et prouvé hors-ligne.
