...
- **Révision des normes différées (§3.3)** : **ISO 5230 — déclencheur ATTEINT** (licence MIT fixée : `LICENSE`, `package.json`, `README`) → contrôle de compat des licences de deps à faire (P4) ; **ISO 23894** en approche (eval gates = contrôle de risque *de facto*, pas de registre formel) → P5 si industrialisation ; **ISO 25024** reste différée.
- **Suite (lot distinct, postérieur au constat figé)** : remédiation **P1** (corriger la décomposition ISO 25012 « 5+2 » → 7 caractéristiques avec lieu d'encodage exact ; par **nouvelle entrée `[Unreleased]`**, jamais réécrire `[0.2.0]`) + **P2** (aligner `ARCHITECTURE.md:67` sur « backbone d'un workflow »), référençant l'audit v1.
