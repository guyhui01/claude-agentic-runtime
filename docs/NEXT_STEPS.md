...
- ▶️ **§2.4-B.3 — run live de la spine (À FAIRE, sous condition)** : remplacer le runner mock par un adaptateur enveloppant `query()` et dérouler la spine pour de vrai.
  ...
  - ▶️ **Reste à faire pour le run live** : (1) **sidecar réel** des 3 agents depuis `claude-agents` (générateur §2.3 — chantier de l'AUTRE repo) ; en attendant, sidecar intérimaire dans `test/spine-wf-001.test.ts`. (2) Câbler `createQueryRunner` dans `runSpine` sur la spine WF-001 + **lancer le run de bout en bout sur accord explicite Guy + run observé** (auth + caps ci-dessous).
